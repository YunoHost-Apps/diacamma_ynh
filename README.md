<!--
N.B.: This README was automatically generated by <https://github.com/YunoHost/apps_tools/blob/main/readme_generator>
It shall NOT be edited by hand.
-->

<h1>
  <img src="https://raw.githubusercontent.com/YunoHost/apps/main/logos/diacamma.png" width="32px" alt="Logo of Diacamma">
  Diacamma, packaged for YunoHost
</h1>

Management application for volunteers (association, condominium)

[![🌐 Official app website](https://img.shields.io/badge/Official_app_website-darkgreen?style=for-the-badge)](https://www.diacamma.org)
[![App Demo](https://img.shields.io/badge/App_Demo-blue?style=for-the-badge)](https://asso.diacamma.org)
[![Version: 25.08.18.09~ynh1](https://img.shields.io/badge/Version-25.08.18.09~ynh1-rgb(18,138,11)?style=for-the-badge)](https://ci-apps.yunohost.org/ci/apps/diacamma/)

<div align="center">
<a href="https://apps.yunohost.org/app/diacamma"><img height="100px" src="https://github.com/YunoHost/yunohost-artwork/raw/refs/heads/main/badges/neopossum-badges/badge_more_info_on_the_appstore.svg"/></a>
<a href="https://github.com/YunoHost-Apps/diacamma_ynh/issues"><img height="100px" src="https://github.com/YunoHost/yunohost-artwork/raw/refs/heads/main/badges/neopossum-badges/badge_report_an_issue.svg"/></a>
</div>


## Screenshots
![Screenshot of Diacamma](./doc/screenshots/01_fiche_adherent.png)
![Screenshot of Diacamma](./doc/screenshots/02_situation_coporprietaire.png)
![Screenshot of Diacamma](./doc/screenshots/03_bilan_comptable.png)

## 📦 Developer info

[![Automatic tests level](https://apps.yunohost.org/badge/cilevel/diacamma)](https://ci-apps.yunohost.org/ci/apps/diacamma/)

🛠️ Upstream Diacamma repository: <https://github.com/Diacamma2>

Pull request are welcome and should target the [`testing` branch](https://github.com/YunoHost-Apps/diacamma_ynh/tree/testing).

The `testing` branch can be tested using:
```
# fresh install:
sudo yunohost app install https://github.com/YunoHost-Apps/diacamma_ynh/tree/testing

# upgrade an existing install:
sudo yunohost app upgrade diacamma -u https://github.com/YunoHost-Apps/diacamma_ynh/tree/testing
```

### 📚 App packaging documentation

Please see <https://doc.yunohost.org/packaging_apps> for more information.